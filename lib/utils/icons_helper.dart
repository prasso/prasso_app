/// a file taken from dynamic_widget library. to correct a build error,
/// it is the only file we use from that package
///
...
IconData? getIconUsingPrefix({required String name}) {
  ...
  if (split.length > 1) {
    ...
    if (split[0].toLowerCase() == 'fa' ||
      ...
      return getFontAwesomeIcon(name: sname);
    }
  } else {
...
String exportIconGuessFavorMaterial(IconData iconData) {
  ...
  for (final entry in FontAwesomeIconsMap.entries) {
    if (entry.value == iconData) {
      return entry.key;
    }
  }
